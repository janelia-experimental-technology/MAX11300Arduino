...
// versions
// 1.0 - 12dec2023 sws
// - renamed functions to mimic Arduino names
...
{
//	if (_interrupt < 255) {attachInterrupt(_interrupt, MAX11300::serviceInterrupt, FALLING);}  // to be implemented and tested
...
bool MAX11300::pinMode(uint8_t pin, pinMode_t mode) 
{
	return this->pinModeMax(pin, mode, 255);
}

bool MAX11300::pinModeMax(uint8_t pin, pinMode_t mode, uint8_t differentialPin) 
{
...
{
	return analogWrite(pin, voltage);
}
...
uint16_t MAX11300::analogRead(uint8_t pin) 
{
	switch(getADCmode()) 
	{
		case Idle:
...
bool MAX11300::digitalRead(uint8_t pin) 
{
...
bool MAX11300::analogWrite(uint8_t pin, uint16_t value) 
{
	return writeRegister((MAX_DACDAT_BASE + pin), value);
}

bool MAX11300::digitalRange( uint8_t pin, float volts)  // set '1' voltage value for digital out
{
	bool status = false;
	if( (volts <= 10.0) && (volts >= 0) ) 
	{
		uint16_t val = (uint16_t) ( 4095.0 * volts / 10.0);
		status =  analogWrite( pin, val);	
		delay(1);
	}
	return status;	
}	

bool MAX11300::digitalWrite(uint8_t pin, uint8_t value) 
{
	uint8_t address, offset;
	uint16_t reg;
	if (pin > 15) 
...
bool MAX11300::ADCreference (uint8_t pin, ADCref_t reference) 
{
	return readModifyWriteRegister((MAX_FUNC_BASE + pin), MAX_FUNCPRM_AVR_MASK, (uint16_t)(reference));
}

ADCref_t MAX11300::getADCreference (uint8_t pin) 
{
	uint16_t reg = readRegister(MAX_FUNC_BASE + pin);
...
bool MAX11300::DACreference(DACref_t reference) 
{
	return readModifyWriteRegister(MAX_DEVCTL, (1 << MAX_DACREF), (uint16_t)(reference));
}

DACref_t MAX11300::getDACreference (void) 
{
...
bool MAX11300::isAnalogConversionComplete (void) 
{
	...
	else if (readRegister(MAX_INT) & MAX_ADCFLAG_MASK) 
    {		
		return true;
	}		
	return false;
...
uint16_t MAX11300::readMaxRegister(uint8_t reg)
{
...
bool MAX11300::DACrange(uint8_t pin, DACRange_t voltRange) 
{
	uint16_t configuration;									
	configuration = readRegister(MAX_FUNC_BASE + pin);	//read in the existing configuration so we don't stomp it
...
bool MAX11300::ADCrange(uint8_t pin, ADCRange_t voltRange) 
{
	uint16_t configuration;									
	configuration = readRegister(MAX_FUNC_BASE + pin);	//read in the existing configuration so we don't stomp it
